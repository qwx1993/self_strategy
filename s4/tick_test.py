...
class TickTest():
    ...
    move_stop_bool = False # False关闭移动止盈，True开启移动止盈
    open_price_effective_ir_last_datetime = None
        ...
        if minute_cd is not None:
            ...
            if self.trade_action is None and trade.simulation_can_open_a_position(self.vt_symbol, tick):
                ...
                if self.history.last_cd is not None:
                    ...
                    current_ir = None
                if self.history.effective_extremum_d_price is not None and S4Tick.open_a_price_by_effective_ir_last(direction, self.history.effective_ir_last, tick_obj):
                    # 时间间隔起点
                ...
                # 移动平仓
                    ...
                    # self.log_obj.info(f"vt_symbol:{self.vt_symbol} => set_agreement_close_price_by_long => tick_last_price:{tick.current} => close_price => {self.close_price} => agreement_close_price:{self.agreement_close_price} => last_cd:{self.history.last_cd}")
                elif False and self.is_exceed_last_cd_low(tick) and self.move_stop_bool:
                ...
                if False and S4Tick.close_a_price_by_breakthrough_ir_last(self.trade_action, current_ir, self.open_price_ir_last, self.unit_value):
                 ...
                 # 移动平仓
                    ...
                    # self.log_obj.info(f"vt_symbol:{self.vt_symbol} => set_agreement_close_price_by_short => tick_last_price:{tick.current} => close_price => {self.close_price} => agreement_close_price:{self.agreement_close_price} => last_cd:{self.history.last_cd}")
                elif False and self.is_exceed_last_cd_low(tick) and self.move_stop_bool:
                    self.set_close_price_by_agreement()
                    # self.log_obj.info(f"vt_symbol:{self.vt_symbol} => set_close_price_by_agreement => tick_last_price:{tick.current} => close_price => {self.close_price} => agreement_close_price:{self.agreement_close_price} => last_cd:{self.history.last_cd}")
    """
    ...
    def on_bar(self, cd):
        ...
        if not self.d_win_flag:
            ...
            if self.trade_action == Cons.ACTION_CLOSE_LONG:
                if S4Tick.close_a_price_by_cd(self.trade_action, self.close_price, cd) or trade.simulation_need_close_position(self.vt_symbol, cd, type='minute') or effective_breakthrough_bool or is_close_by_duration:
                    # result = self.sell(tick.current, self.hand_number)
                    self.add_action(cd, Cons.ACTION_CLOSE_LONG, cd.close - self.unit_value)
                    self.log_obj.info(f"vt_symbol => {self.vt_symbol} \nclose_direction => long \nclose_price => {self.close_price} \ncd_close => {cd.close} \ncd => {cd} \neffective_breakthrough_bool => {effective_breakthrough_bool} \nis_close_by_duration => {is_close_by_duration}")
    ...
    def after_close_for_cd(self, cd):
        ...
        self.close_by_refresh_agreement_ir = False # 平仓后设置为False
        self.same_direction_ir = []
        self.interval_minutes_count = 0 
        self.move_stop_bool = False # 移动止盈设置为False
        # 讲回归状态设置为None
        if self.history.effective_ir_last is not None and (self.open_price_effective_ir_last_datetime == self.history.effective_ir_last.datetime):
            if self.history.effective_ir_last.status == Cons.IR_LAST_FALLBACK:
                self.history.effective_ir_last.status = Cons.IR_LAST_NONE
    ...
    def after_open_a_position(self):
        ...
        self.interval_minutes_count = 0
        self.open_price_effective_ir_last_datetime = self.history.effective_ir_last.datetime
    ...
    在指定分钟后平仓
    """
    def is_close_by_duration(self):
        if self.trade_action is not None:
            self.interval_minutes_count += 1
            if self.interval_minutes_count > 60:
                return True
        return False
