...
class DemoStrategy(CtaTemplate):
    ...
    def on_bar(self, bar: BarData):
        ...
        cross_below = (self.fast_ma0 < self.slow_ma0 and
                       self.fast_ma1 > self.slow_ma1)
        self.write_log(f"开仓测试。。。")
        test_price = bar.close_price + 5
        self.buy(price, 1)
        self.cover(price, 1)
        self.write_log(f"平仓结束。。。")
