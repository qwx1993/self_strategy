...
class AnalysisTwo:
    ...
    action_record = None  # 交易动作记录
    s2_action_record = None  # 交易动作记录
    ...
    def set_max_l_to_d_interval_data(self, cd):
        current_l_to_d_interval = abs(cd.high - cd.low)
    ...
    def set_max_r(self, cd, is_same_direction):
        if self.max_r is None:
            self.max_r = Logic.amplitude_length(self.extremum_d, self.breakthrough_direction)
        else:
            if is_same_direction:
                decrease_range = Logic.amplitude_length(cd, self.breakthrough_direction)
                if self.max_r < decrease_range:
                    self.max_r = decrease_range
            else:
                max_decrease_range = Logic.max_amplitude_length(cd)
                if self.max_r < max_decrease_range:
                    self.max_r = max_decrease_range
    ...
    def counter_trend(self, cd, temp_file, line):
        if Logic.is_counter_trend(cd, self.max_l_to_d_interval, self.breakthrough_direction):
            ...
            self.set_s2_status(Constants.SITUATION_TWO_REVERSE_OPEN_A_POSITION)
